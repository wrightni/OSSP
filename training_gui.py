...
from lib import feature_calculations
	...
	def add_segments(self):
		if self.mode == 1:
			...
			# while j < 200:
				...
				# tried less than 10 subimage so far, try another one.
				if len(segments_to_add)<7 and j<10:
					j+=1
				...
				while (len(segments_to_add)<10 and j<200) and len(segments_to_add)<np.amax(ws_subimage):
					...
					if sp_size > 10 and intensity >= 1:
						...
						if [self.image_name, next_subimage, i] not in segments_to_add:
							segments_to_add.append([self.image_name, next_subimage, i])
						else:
							j+=1
					else:
...
if __name__ == "__main__":
	...
	# FILE_PATH is the path of either the training dataset or the accuracy
	...
	if mode == 1:
		...
		while image_list:
			...
			for next_image in image_list:
				...
				# If the provided folder already contains splits, we can proceed to use those directly
				if image_name[-13:] == '_segmented.h5':
					segmented_name = image_name
					split_path = FILE_PATH
				elif image_ext.lower() == '.tif' or image_ext.lower() == '.jpg':

					# Look for the segmented version of the image
					split_path = os.path.join(FILE_PATH, "training_images")
					split_name = image_root + "_s01of{0:02d}.h5".format(number_of_splits)
					segmented_name = os.path.splitext(split_name)[0] + '_segmented.h5'
					
					# If the split directory already exists, look for the right segmented image
					if os.path.isdir(split_path):
						split_candidate = segmented_name
						if os.path.isfile(os.path.join(split_path, split_candidate)):
							found_split = True
						else:
							found_split = False
					...
					if found_split is False:
						print "Preparing provided images..."
						split_image(FILE_PATH, image_name, im_type, output_path=split_path, number_of_splits=number_of_splits)
						segment_image(split_path, split_name, output_path=split_path)
				# Remove from the list if it is neither a valid image or segmented image
				else:
					image_list.remove(image_name)
					continue

				try:
					image_file = h5py.File(os.path.join(split_path, segmented_name), 'r') #band_id: ([subimage][row][column])
				except IOError:
					print "Cannot locate %s.h5" %segmented_name
					continue
				original_image = [] 	#[subimage:row:column:band]
				band_1 = image_file['original_1'][:]
				watershed_image = image_file['watershed'][:]
				# im_type = image_file.attrs.get('Image Type')
				im_date = image_file.attrs.get('Image Date')

				# ADD: compare imtype attrs to user entered imtype
				# im_type = 'srgb'

				# Adds remaining 7 bands if they exist
				if im_type == 'wv02_ms':

					band_2 = image_file['original_2'][:]
					band_3 = image_file['original_3'][:]
					band_4 = image_file['original_4'][:]
					band_5 = image_file['original_5'][:]
					band_6 = image_file['original_6'][:]
					band_7 = image_file['original_7'][:]
					band_8 = image_file['original_8'][:]

					for i in range(len(band_1)):
						original_image.append(create_composite([
					...
					original_image = band_1

				image_file.close()

				print image_name
				#### Initializing the GUI
				tW = TrainingWindow(original_image, watershed_image, segment_list, image_name, label_vector, feature_matrix, im_type, im_date, mode, input_file)
	...
	if mode == 2:
		...
		#### Initializing the GUI
		tW = TrainingWindow(original_image, classified_image, segment_list, image, label_vector, feature_matrix, im_type, im_date, mode, savepath)
