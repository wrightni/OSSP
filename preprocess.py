...
def prepare_image(input_path, image_name, image_type, 
    ...
    Image splits are saved as .h5 formatted datasets.
        Attrs: {[Image Date], [Image Type], [Diminsions]}
    ...
    #   values between iterations, and can proceed to stretching and 
    #   saving the current band directly. 
    if image_type == 'wv02_ms' or image_type == 'pan':
        sat_output_bands = {} # {band_id: [subimage][row][column]}
        for b in range(1,BANDS+1):
            # Read the band information from the gdal dataset
            if verbose: print "Reading band %s data..." %b
            band = dataset.GetRasterBand(b)
            ...
            lower_b, upper_b = find_threshold(hist, bin_centers, 
                                              peaks, image_type)
            # Unlikely, but construct image blocks and compile all bands to
            #   return
            if number_of_splits == 1:
                if verbose: print "Rescaling band %s" %b
                # Rescale the band
                ...
                if verbose: print "Splitting band %s..." %b
                sat_band_split = split_band(band, x_splits, y_splits, 
                                            split_cols, split_rows)
                band = None
                snum = 1
                for split in sat_band_split:
                    # Rescale the split
                    sat_split_rescaled = rescale_band(split, lower_b, upper_b)
                    split = None
                    # Grid the split
                    sat_split_blocked,dimensions = construct_blocks(
                                                    sat_split_rescaled, 
                                                    block_cols, 
                                                    block_rows,
                                                    [split_rows,split_cols])
                    sat_split_rescaled = None
                    # Determine the output filename
                    fname = (os.path.splitext(image_name)[0] 
                            + "_s{0:02d}of{1:02d}.h5".format(snum, 
                                                             number_of_splits))
                    dst_file = os.path.join(output_path, fname)
                    # Save the split
                    write_to_hdf5(dst_file, sat_split_blocked, b, image_type, 
                                  image_date, dimensions)
                    snum += 1
        ...
        dataset = None
        meta_data = [dimensions, image_date]
        return sat_output_bands, meta_data
...
def find_peaks(hist, bin_centers, image_type):
    ...
    Criteria for each peak:
        Distance to the nearest neighboring peak is greater than one third 
            the approx. dynamic range of the input image
        Has a minimum number of pixels in that peak, loosely based on image size
        Is greater than the directly adjacent bins, and the bins +/- 5 bins away
    '''

    # Roughly define the smallest acceptable size of a peak based on the
    #  input image type. 
    if image_type == 'srgb':
        min_count = 1000
    else:
...
def find_threshold(hist, bin_centers, peaks, image_type):
    ...
    Using the indicies of the highest and lowest peak (by intensity, 
        not # of pixels), this searches for an upper threshold that is both 
        greater than the highest peak and has fewer than 10% the number of 
        pixels, and a lower threshold that is less than the lowest peak and 50%
    ...
    #   in the case that all three main surface types are not represnted (fewer
    #   than 3 peaks)
